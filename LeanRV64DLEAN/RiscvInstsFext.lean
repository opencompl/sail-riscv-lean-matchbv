...
def select_instr_or_fcsr_rm (instr_rm : rounding_mode) : SailM (Option rounding_mode) := do
  if (BEq.beq instr_rm RM_DYN)
  then
    let fcsr_rm ← do (pure (_get_Fcsr_FRM (← readReg fcsr)))
    if (Bool.and (valid_rounding_mode fcsr_rm) (bne fcsr_rm (encdec_rounding_mode_forwards RM_DYN)))
...
def fsplit_S (x32 : (BitVec 32)) : ((BitVec 1) × (BitVec 8) × (BitVec 23)) :=
  let t__3986 := (Sail.BitVec.extractLsb x32 31 31)
  let t__3987 := (Sail.BitVec.extractLsb x32 30 23)
  let t__3988 := (Sail.BitVec.extractLsb x32 22 0)
  (t__3986, t__3987, t__3988)
...
def feq_quiet_S (v1 : (BitVec 32)) (v2 : (BitVec 32)) : (Bool × (BitVec 5)) :=
  ...
  let fflags :=
    if (Bool.or (f_is_SNaN_S v1) (f_is_SNaN_S v2))
    then (nvFlag ())
    else (zeros_implicit (n := 5))
  (result, fflags)

/-- Type quantifiers: k_ex313853# : Bool -/
def flt_S (v1 : (BitVec 32)) (v2 : (BitVec 32)) (is_quiet : Bool) : (Bool × (BitVec 5)) :=
  ...
  let fflags :=
    ...
    then
      ...
      if (Bool.or (f_is_NaN_S v1) (f_is_NaN_S v2))
      then (nvFlag ())
      else (zeros_implicit (n := 5))
  (result, fflags)

/-- Type quantifiers: k_ex313921# : Bool -/

import LeanRV64DLEAN.RiscvSmcntrpmf
...
/-- Type quantifiers: k_ex317207# : Bool -/
...
/-- Type quantifiers: k_ex317294# : Bool -/
...
/-- Type quantifiers: k_ex317770# : Bool -/
...
def getPendingSet (priv : Privilege) : SailM (Option ((BitVec (2 ^ 3 * 8)) × Privilege)) := do
  ...
  bif (Bool.and mIE (bne pending_m (zeros_implicit (n := ((2 ^i 3) *i 8)))))
  then (pure (some (pending_m, Machine)))
  else
    (bif (Bool.and sIE (bne pending_s (zeros_implicit (n := ((2 ^i 3) *i 8)))))
...
/-- Type quantifiers: k_ex318016# : Bool -/

import LeanRV64DLEAN.RiscvInstsFext
...
def fsplit_D (x64 : (BitVec 64)) : ((BitVec 1) × (BitVec 11) × (BitVec 52)) :=
  let t__3815 := (Sail.BitVec.extractLsb x64 63 63)
  let t__3816 := (Sail.BitVec.extractLsb x64 62 52)
  let t__3817 := (Sail.BitVec.extractLsb x64 51 0)
  (t__3815, t__3816, t__3817)
...
def f_is_neg_inf_D (x64 : (BitVec 64)) : Bool :=
  ...
  (Bool.and (BEq.beq sign (0b1 : (BitVec 1)))
    (Bool.and (BEq.beq exp (ones (n := 11))) (BEq.beq mant (zeros_implicit (n := 52)))))

def f_is_neg_norm_D (x64 : (BitVec 64)) : Bool :=
  let (sign, exp, mant) := (fsplit_D x64)
  (Bool.and (BEq.beq sign (0b1 : (BitVec 1)))
    (Bool.and (bne exp (zeros_implicit (n := 11))) (bne exp (ones (n := 11)))))

def f_is_neg_subnorm_D (x64 : (BitVec 64)) : Bool :=
  let (sign, exp, mant) := (fsplit_D x64)
  (Bool.and (BEq.beq sign (0b1 : (BitVec 1)))
    (Bool.and (BEq.beq exp (zeros_implicit (n := 11))) (bne mant (zeros_implicit (n := 52)))))

def f_is_neg_zero_D (x64 : (BitVec 64)) : Bool :=
  let (sign, exp, mant) := (fsplit_D x64)
  (Bool.and (BEq.beq sign (ones (n := 1)))
    (Bool.and (BEq.beq exp (zeros_implicit (n := 11))) (BEq.beq mant (zeros_implicit (n := 52)))))
...
def f_is_SNaN_D (x64 : (BitVec 64)) : Bool :=
  ...
  (Bool.and (BEq.beq exp (ones (n := 11)))
    (Bool.and (BEq.beq (BitVec.access mant 51) 0#1) (bne mant (zeros_implicit (n := 52)))))
...
def f_is_NaN_D (x64 : (BitVec 64)) : Bool :=
  let (sign, exp, mant) := (fsplit_D x64)
  (Bool.and (BEq.beq exp (ones (n := 11))) (bne mant (zeros_implicit (n := 52))))
...
/-- Type quantifiers: k_ex315184# : Bool -/
...
/-- Type quantifiers: k_ex315252# : Bool -/

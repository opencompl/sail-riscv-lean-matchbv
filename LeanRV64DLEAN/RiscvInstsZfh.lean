import LeanRV64DLEAN.RiscvInstsZbs
...
def fsplit_H (xf16 : (BitVec 16)) : ((BitVec 1) × (BitVec 5) × (BitVec 10)) :=
  let t__3759 := (Sail.BitVec.extractLsb xf16 15 15)
  let t__3760 := (Sail.BitVec.extractLsb xf16 14 10)
  let t__3761 := (Sail.BitVec.extractLsb xf16 9 0)
  (t__3759, t__3760, t__3761)
...
def f_is_neg_inf_H (xf16 : (BitVec 16)) : Bool :=
  ...
  (Bool.and (BEq.beq sign (0b1 : (BitVec 1)))
    (Bool.and (BEq.beq exp (ones (n := 5))) (BEq.beq mant (zeros_implicit (n := 10)))))

def f_is_neg_norm_H (xf16 : (BitVec 16)) : Bool :=
  let (sign, exp, mant) := (fsplit_H xf16)
  (Bool.and (BEq.beq sign (0b1 : (BitVec 1)))
    (Bool.and (bne exp (zeros_implicit (n := 5))) (bne exp (ones (n := 5)))))

def f_is_neg_subnorm_H (xf16 : (BitVec 16)) : Bool :=
  let (sign, exp, mant) := (fsplit_H xf16)
  (Bool.and (BEq.beq sign (0b1 : (BitVec 1)))
    (Bool.and (BEq.beq exp (zeros_implicit (n := 5))) (bne mant (zeros_implicit (n := 10)))))
...
def f_is_neg_zero_H (xf16 : (BitVec 16)) : Bool :=
  ...
  (Bool.and (BEq.beq sign (ones (n := 1)))
    (Bool.and (BEq.beq exp (zeros_implicit (n := 5))) (BEq.beq mant (zeros_implicit (n := 10)))))
...
def f_is_SNaN_H (xf16 : (BitVec 16)) : Bool :=
  ...
  (Bool.and (BEq.beq exp (ones (n := 5)))
    (Bool.and (BEq.beq (BitVec.access mant 9) 0#1) (bne mant (zeros_implicit (n := 10)))))
...
def f_is_NaN_H (xf16 : (BitVec 16)) : Bool :=
  let (sign, exp, mant) := (fsplit_H xf16)
  (Bool.and (BEq.beq exp (ones (n := 5))) (bne mant (zeros_implicit (n := 10))))

/-- Type quantifiers: k_ex318133# : Bool -/
